...
# Operator password-login (parallel to admin password_login.py).  File is
# generated by scripts/generate-operator-passwords.sh from
# config.local.yaml::auth.operator_emails.  Path resolved at call time so
# tests can monkeypatch via chdir.
_OPERATOR_PASSWORDS_FILE = Path(".autoservice") / "operator_passwords.json"

# Rate-limit state for password-login (5 failures / 10 min / IP).  In-memory
# only — process restart resets it.  Mirrors admin password_login.py.
_PW_FAILED_ATTEMPTS: dict[str, list[float]] = defaultdict(list)
_PW_WINDOW_SECONDS = 10 * 60
_PW_MAX_FAILURES = 5

# Dummy bcrypt hash kept hot so the constant-time check on missing emails
# does not leak membership via timing.  Generated lazily on first need.
_PW_DUMMY_HASH = bcrypt.hashpw(b"unused-reference", bcrypt.gensalt(4)).decode()
...
# ──────────────────────────────────────────────────────────────────────────
# Password login (production fallback when AUTH_DEV_MODE=0)
# ──────────────────────────────────────────────────────────────────────────
#
# Mirrors admin ``password_login.py`` for the operator surface.  Reads
# bcrypt hashes from ``.autoservice/operator_passwords.json`` (generated
# by ``scripts/generate-operator-passwords.sh``).  Same security shape as
# the admin path: constant-time bcrypt comparison even on missing emails,
# rate-limited per client IP.
#
# On success runs the same operator upsert + ``operator_session`` mint as
# dev-login, so the WS handshake (T1S.3 strict cookie validation) treats
# the resulting cookie identically.
#
# Disabled when the passwords file is missing — the endpoint then 404s,
# matching the admin convention of not advertising surface area.


def _load_operator_password_entries() -> list[dict] | None:
    """Return the entries list from operator_passwords.json, or None if disabled.

    Returns None when the file is missing OR malformed; callers should
    treat that as "feature off" and respond 404.
    """
    p = _OPERATOR_PASSWORDS_FILE
    if not p.exists():
        return None
    try:
        data = json.loads(p.read_text(encoding="utf-8"))
    except (json.JSONDecodeError, OSError) as exc:
        logger.warning("[operator-password-login] cannot read %s: %s", p, exc)
        return None
    entries = data.get("entries") or []
    return entries if isinstance(entries, list) else None


def _find_operator_password_entry(
    entries: list[dict], email: str
) -> dict | None:
    target = email.lower()
    for e in entries:
...
def _pw_is_locked(ip: str) -> bool:
    now = time.time()
    _PW_FAILED_ATTEMPTS[ip] = [
        t for t in _PW_FAILED_ATTEMPTS.get(ip, []) if (now - t) < _PW_WINDOW_SECONDS
    ]
    return len(_PW_FAILED_ATTEMPTS[ip]) >= _PW_MAX_FAILURES


def _pw_record_failure(ip: str) -> None:
    _PW_FAILED_ATTEMPTS[ip].append(time.time())


def _pw_clear_failures(ip: str) -> None:
    _PW_FAILED_ATTEMPTS.pop(ip, None)


@operator_router.post("/auth/operator/password-login")
async def operator_password_login(
    request: Request,
    payload: dict[str, Any] = Body(...),
) -> Any:
    """Production-safe operator login: email + password + tenant_id.

    Request body::

        {"email": "<op@example.com>", "password": "<pw>", "tenant_id": "<tid>"}
    ...
    Errors:
      * 404 — passwords file missing (feature disabled).
      * 400 — required field missing / blank.
      * 429 — too many failures from this IP (5 / 10 min).
      * 401 — unknown email or wrong password OR operator disabled.
    """
    entries = _load_operator_password_entries()
    if entries is None:
        return JSONResponse(status_code=404, content={"error": "not found"})

    email_raw = payload.get("email") if isinstance(payload, dict) else None
    pw_raw = payload.get("password") if isinstance(payload, dict) else None
    tenant_raw = payload.get("tenant_id") if isinstance(payload, dict) else None
    if not isinstance(email_raw, str) or not email_raw.strip():
        return JSONResponse(
            status_code=400, content={"error": "email is required"}
        )
    if not isinstance(pw_raw, str) or not pw_raw:
        return JSONResponse(
    ...
    if _pw_is_locked(ip):
        logger.warning("[operator-password-login] ip=%s locked out", ip)
        return JSONResponse(
    ...
    entry = _find_operator_password_entry(entries, email)
    stored = str(entry.get("password_bcrypt", "")) if entry is not None else ""
    # Constant-time bcrypt against a dummy when email unknown so timing
    # cannot leak membership.
    check_against = stored if stored else _PW_DUMMY_HASH
    valid = bcrypt.checkpw(pw_raw.encode(), check_against.encode())
    if entry is None or not valid:
        _pw_record_failure(ip)
        logger.warning(
            "[operator-password-login] ip=%s email=%s bad credentials", ip, email
        )
        return JSONResponse(
    ...
    op = operators.get_operator_by_email(conn, tenant_id, email)
    if op is None:
        # First successful login on this tenant — auto-create as responder
        # so the operator can log in without a separate provisioning step.
        # Matches dev-login semantics; admin CRUD can later promote roles.
        op = operators.create_operator(
            conn, tenant_id=tenant_id, email=email, role="responder"
        )
    elif op.status != "active":
        # Don't count as a credential failure (the password was valid),
        # but still refuse the session.
        return JSONResponse(
            status_code=401, content={"error": "operator disabled"}
        )

    _pw_clear_failures(ip)
    ...
    tok_prefix = op_token[:8]
    logger.info(
        "[operator-password-login] minted session for %s (tenant=%s, op=%s, tok=%s…)",
        email, tenant_id, op.id, tok_prefix,
    )
