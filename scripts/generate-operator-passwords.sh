#!/usr/bin/env bash
# For every email in .autoservice/config.local.yaml::auth.operator_emails
# that does NOT yet have an entry in .autoservice/operator_passwords.json,
...
#
# Mirrors scripts/generate-admin-passwords.sh — keep them in sync.
set -euo pipefail

REPO="$(cd "$(dirname "$0")/.." && pwd)"
CFG="$REPO/.autoservice/config.local.yaml"
PWFILE="$REPO/.autoservice/operator_passwords.json"
...
while IFS= read -r line; do
  emails+=("$line")
done < <(yq e '.auth.operator_emails[]' "$CFG" 2>/dev/null || true)
[[ ${#emails[@]} -gt 0 ]] || {
  echo "No operator_emails in $CFG (add them under auth.operator_emails)"
  exit 4
}
...
if [[ ${#newly_generated_emails[@]} -eq 0 ]]; then
  echo "No new passwords generated — all operator_emails already have entries."
  exit 0
fi

echo
echo "======================================================================"
echo "  NEW OPERATOR PASSWORDS — NOT SHOWN AGAIN. DISTRIBUTE SECURELY."
echo "======================================================================"
...
echo "======================================================================"
echo
echo "Operators login at /console/ — pick the tenant they belong to and"
echo "enter email + password.  First successful login auto-creates the"
echo "operator row with role='responder'."
echo
