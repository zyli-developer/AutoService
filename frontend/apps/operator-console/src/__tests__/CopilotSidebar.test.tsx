...
describe('CopilotSidebar', () => {
  it('TC-01: sidebar not rendered when activeCopilotConvId is null', () => {
    const send = vi.fn();
    render(<CopilotSidebar send={send} />);
    expect(screen.queryByTestId('copilot-sidebar')).toBeNull();
  });

  it('TC-02: sidebar rendered when activeCopilotConvId is set', () => {
  ...
  it('TC-03: messages displayed in order', () => {
    ...
    // Verify order: msg-1 comes before msg-2 in DOM
  ...
  it('TC-04: typing in input and clicking send calls the send callback', async () => {
    useOperatorStore.setState({ activeCopilotConvId: 'conv-1' });
    const send = vi.fn();
