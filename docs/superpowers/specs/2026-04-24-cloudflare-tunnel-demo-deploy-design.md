...
| Admin/operator auth | magic-link + SMTP, **plus narrow password login** | Feishu Mail; password-login only for the shared dev account |
...
### 4.6 Password login (narrowly scoped to shared dev account)

To speed up dev/QA work without waiting on email delivery each
time, we add a password login path that is strictly limited to the
shared admin account.

**Contract:**

- New endpoint: `POST /api/auth/password-login`
  Body: `{"email": str, "password": str}`
  - If `email != autoservice@h2oslabs.com` → 401 (no user enumeration,
    same response as bad password).
  - If bcrypt verify fails → 401 + increment failed-attempt counter.
  - If 5 failed attempts in 10 minutes → 429 for the next 10 minutes.
  - Success → mint the same `auth_session` cookie as magic-link, with
    the same TTL, `HttpOnly`, `Secure`, `SameSite=Lax`, `path=/`.
- Password stored as bcrypt hash (cost 12) in
  `.autoservice/config.local.yaml::auth.shared_admin.password_bcrypt`.
  Initial hash generated by `scripts/set-shared-admin-password.sh`
  prompting interactively; never logged in plaintext.
- Absent `auth.shared_admin.password_bcrypt` in config → endpoint
  returns 404 (feature disabled). This is the behavior on a fresh
  clone where no password has been set.

**Frontend** (`admin-portal/src/components/auth/LoginPage.tsx`):
- Keep magic-link as the primary action (top of the form).
- Add a collapsed section "Developer password login" below, expanded
  only when the user clicks. Email is pre-filled with
  `autoservice@h2oslabs.com`. Submit posts to `/api/auth/password-login`.

**Rationale:**
- Attack surface is exactly one account; privilege escalation of the
  shared password only gains admin on the demo. CF Access still guards
  the edge, so an attacker must first pass Access OTP.
- The four developer accounts do NOT get password login. They use
  magic-link; Feishu Mail delivery on internal addresses is
  sub-second in practice.
- Full multi-user password login is explicitly out of scope; upgrade
  path is straightforward if needed later.

**Operator console** (`/console/*`) does NOT get password login —
magic-link only. Rationale: operator accounts are per-tenant and
per-user; a shared password would defeat attribution.
...
auth:
  ...
  smtp:
    ...
    starttls: true
  shared_admin:
    # Populated by scripts/set-shared-admin-password.sh. When present,
    # /api/auth/password-login accepts autoservice@h2oslabs.com + the
    # verified password. When absent, that endpoint returns 404.
...
### 7.5 `scripts/set-shared-admin-password.sh`

Interactive bcrypt helper:
1. Prompt for password (twice, hidden input via `read -s`).
2. Use `python -c 'import bcrypt, sys; print(bcrypt.hashpw(sys.stdin.read().strip().encode(), bcrypt.gensalt(12)).decode())'`
   to compute the hash.
3. Patch `.autoservice/config.local.yaml::auth.shared_admin.password_bcrypt`
   in place (via `yq` or `ruamel.yaml`).
4. Emit "done; restart gateway with `make public-reload` to load".
...
| `set-admin-password` | Interactive wrapper for `scripts/set-shared-admin-password.sh` |
...
- `scripts/set-shared-admin-password.sh`
...
- `Makefile` (new public-* targets + access-apply + set-admin-password)
