...
| Admin/operator auth | magic-link + SMTP (A2 full public) | Feishu Mail |
| Dangerous-endpoint gate | Cloudflare Access policy (X) | path-selective list below |
...
The Access application is configured in the Cloudflare dashboard
(Zero Trust → Access → Applications); policy-as-code is out of scope
for a demo. A runbook entry documents exact paths and the identity
provider choice.
...
### 7.4 SMTP — `.autoservice/config.local.yaml`
...
auth:
  admin_emails:
    ...
    # add trial-customer admin emails here
  smtp:
    host: smtp.feishu.cn
    port: 587
...
- `autoservice/web_gateway.py` (CORS_EXTRA_ORIGINS)
- `Makefile` (new public-* targets)
